    ...
    with col_in:
        ...
        if is_prop_calc:
            ...
            calc_mde_type_c = st.radio(
                "MDE expressed as",
                ["Relative % of mean", "Absolute units"],
                horizontal=True,
                key="calc_mde_type_c",
            )
    ...
    with col_out:
        if not valid:
            ...
            if is_prop_calc:
                ...
                st.info(
                    f"Detects **{calc_baseline:.2%} → {target:.2%}** "
            ...
            with c1:
                st.plotly_chart(
                    power_curve(eff, calc_alpha, n, calc_power),
                    use_container_width=True,
                )
            with c2:
                if is_prop_calc:
                    st.plotly_chart(
                        mde_curve(calc_baseline, calc_alpha, calc_power, n),
                        use_container_width=True,
                    )
                else:
