...
with st.form("rca_form"):
    ...
    with c1:
        metric_name = st.text_input(
            "Metric name", value=default_metric,
            placeholder="e.g. 7-day retention, checkout conversion rate, DAU/MAU",
        )
    with c2:
        direction = st.selectbox("Direction", ["dropped", "increased unexpectedly", "is volatile / erratic"])

    c3, c4 = st.columns(2)
    with c3:
        magnitude = st.text_input("Magnitude", value=default_magnitude, placeholder="e.g. -8% relative, 32% → 29%")
    with c4:
        time_period = st.text_input("When / time window", placeholder="e.g. last Monday, 7-day rolling vs prior period")

    product_context = st.text_area(
        "Product / platform context", value=default_context,
        placeholder="e.g. iOS checkout flow, B2B SaaS dashboard, marketplace for freelancers", height=75,
    )
    known_events = st.text_area(
        "Known events around this time",
        placeholder="e.g. deployed v4.2 on Friday, ran email campaign, Thanksgiving week", height=75,
    )
...
if submitted:
    ...
    segments = [
        label for flag, label in [
            (seg_all, "all users"), (seg_new, "new users"), (seg_returning, "returning users"),
            (seg_mobile, "mobile"), (seg_desktop, "desktop"), (seg_geo, "specific geo"),
            (seg_cohort, "specific cohort"), (seg_unknown, "not yet investigated"),
        ] if flag
    ]
    ...
    tab_dq, tab_seg, tab_deploy = st.tabs([
        "1. Data Quality / Pipeline", "2. Segment Breakdown", "3. Deploy / Event Impact"
    ])
    with tab_dq:
    ...
    with st.spinner("Claude is building your hypothesis matrix..."):
        try:
            analysis = analyze_metric_movement(
                metric_name=metric_name, movement_direction=direction, magnitude=magnitude,
                time_period=time_period or "not specified", product_context=product_context or "not specified",
                known_events=known_events, affected_segments=segments,
            )
    ...
    st.page_link("pages/3_Metric_Tradeoffs.py", label="⚖️ Analyze trade-offs for the proposed fix →")

    handoff_out = {
        "source": "root_cause_analysis", "metric": metric_name, "movement": direction,
        "magnitude": magnitude, "context": product_context, "known_events": known_events,
    }
    with st.expander("Export context for Metric Trade-offs tool"):
        st.code(json.dumps(handoff_out, indent=2), language="json")
        st.caption("Copy this JSON and paste it into the Metric Trade-offs tool.")
